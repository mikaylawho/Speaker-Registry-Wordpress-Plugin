...
?>
	...
	<div class="wrap">
		...
		<p>Choose a CiviMember Membership Type and a Wordpress Role below. This will associate that Membership with the
			Role. If you would like the have the same Membership be associated with more than one role, you will need to
			add a second association rule after you have completed this one.</p>
				...
				<tr class="form-field form-required">
					...
					<td>
						<select name="wp_role" id="wp_role" class="required">
							<option value=""></option>
							<?php
							//added global var for $wp_role
							global $wp_roles, $wp_role;
							$roles = $wp_roles->get_names();
							foreach ( $roles as $key => $value ) {
								?>
								<option value=<?php echo $value;
								if ($value == $wp_role) { ?> selected="selected" <?php } ?>> <?php echo $value; ?></option>
							<?php } ?>
						</select>
					</td>
				</tr>
				...
				<tr>
					...
					<td id="expire-status-td">
						...
						foreach ( $MembershipStatus as $key => $value ) {
							?>
							<input type="checkbox" name=<?php echo "expire[$key]"; ?> id=<?php echo "expire[$key]"; ?>
							       value=<?php echo $key;
				...
				<tr class="form-field form-required">
					...
					<td>
						<select name="expire_assign_wp_role" id="expire_assign_wp_role" class="required">
							<option value=""></option>
							<?php global $wp_roles, $expired_wp_role;
							$roles = $wp_roles->get_names();
							foreach ( $roles as $key => $value ) {
								?>
								<option value=<?php echo $value;
								if ($value == $expired_wp_role) { ?> selected="selected" <?php } ?>> <?php echo $value; ?></option>
							<?php } ?>
						</select>
					</td>
				</tr>
...
if ( $_POST ) {
	...
	if ( ! empty( $_POST['civi_member_type'] ) ) {
		$civi_member_type = utf8_encode( $_POST['civi_member_type'] );
	}
	...
	if ( empty( $sameType ) && empty( $errors ) ) {
		...
		?> <span class="error" style="color: #FF0000;"><?php foreach ( $errors as $key => $values ) {
				echo $values . "<br>";
			} ?> </span>  <?php
	}
}
